...
def show_books_to_delete(_cursor, _user_id):

    query = ("SELECT book_id, book_name, author, details "
            "FROM book "
            "WHERE book_id IN (SELECT book_id FROM wishlist WHERE user_id = {})".format(_user_id))
...
def delete_book_from_wishlist(_cursor, _user_id, _book_id):
    _cursor.execute("DELETE FROM wishlist WHERE user_id = %s and book_id = %s", (_user_id, _book_id))
    ...
    while user_selection != 4:
        ...
        if user_selection == 3:
            ...
            while account_option != 4:
                ...
                # I wanted to see if I could add a delete function, and was successful!
                if account_option == 3:
                    ...
                    delete_book_from_wishlist(cursor, my_user_id, book_id)
